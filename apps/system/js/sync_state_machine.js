...
(function(exports) {
  ...
  BaseModule.create(SyncStateMachine, {
     ...
     * 'A' to state 'B':
     *
     * Service.request('SyncStateMachine:doB').then(() => {...});
     *
     * window.addEventListener('SyncStateMachineB', event => {
    ...
    _createTransition: function(name, event) {
      return () => {
        ...
        // Publish the SyncStateMachineSTATE event.
        var eventName = to.charAt(0).toUpperCase() + to.slice(1);
